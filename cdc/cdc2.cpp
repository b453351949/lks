...
uint64_t hash_func(unsigned char *input, unsigned int pos)
{
	...
	if(hash==0){
		...
		hash=hash*PRIME-input[pos-1]*pow(PRIME,WIN_SIZE+1)+input[pos-1+WIN_SIZE]*PRIME;
...
void cdc(unsigned char *buff, unsigned int buff_size)
{
        ...
        time_cdc2.stop();
        std::cout << "Total latency of cdc1 is: " << time_cdc2.latency() << " ns." << std::endl;
        std::cout << "Average latency of cdc1 per loop iteration is: " << time_cdc2.avg_latency() << " ns." << std::endl;
